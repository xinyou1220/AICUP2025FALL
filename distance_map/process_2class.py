...
def normalize_sdf_clipped(sdf, clip_range=10, feature_range=(-1, 1)):
    ...
    先裁剪到 [-clip_range, clip_range]，再歸一化到 [-1, 1]
    ...
    """
    # 裁剪到指定範圍
    sdf_clipped = np.clip(sdf, -clip_range, clip_range)
    
    # 歸一化到目標範圍
    min_val, max_val = feature_range
    sdf_normalized = sdf_clipped / clip_range  # 先歸一化到 [-1, 1]
...
def main():
    ...
    OUTPUT_DIR = r"C:\Users\user\Desktop\labelsTr\sdfTr2"
    ...
    print("=" * 70)
    print("Distance Map 生成程式 - 多標籤獨立版本")
    print("=" * 70)
    print(f"\nLabel 資料夾: {label_dir}")
    ...
    # 設定裁剪範圍
    clip_range = 10
    print(f"使用方法: Clipped Normalization")
    print(f"裁剪範圍: ±{clip_range} pixels")
    print(f"輸出範圍: [-1, 1]")
