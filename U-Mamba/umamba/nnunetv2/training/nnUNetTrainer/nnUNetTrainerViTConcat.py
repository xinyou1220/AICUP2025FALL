...
class nnUNetWithViTBottleneck(nn.Module):
    def __init__(self, unet_backbone: nn.Module, vit_encoder: nn.Module,
        ...
        if self.mode == "concat":
            self.fuse = nn.Sequential(
                ...
                nn.InstanceNorm3d(bottleneck_channels),
    ...
    def forward(self, x):
        ...
        bottleneck = self.layernorm(bottleneck.permute(0, 2, 3, 4, 1)).permute(0, 4, 1, 2, 3)

        if self.mode == "concat":
            fused = torch.cat([bottleneck, vit_feat], dim=1)
...
class nnUNetTrainerViTlayernorm(nnUNetTrainer):
    def __init__(self,
                 plans: dict,
                 configuration: str,
                 fold: int,
